...
(function() {
  ...
  function notchRow(i, sex = '', renn = '', lenn = '', rn = '', teats = '', weight = '', weights = null, tag = '') {
    const teatOn = sex === 'F';
    ...
    return `<tr data-notch-row class="notch-row"><td class="notch-no">${i}</td>` +
      `<td><input name="notch_tag" inputmode="numeric" autocomplete="off" placeholder="ear tag" value="${esc(tag)}"></td>` +
  ...
  function notchRowsHTML(b) {
    const roster = Array.isArray(b.roster) ? b.roster : [],
      rn = litterNo(b);
    if (roster.length) return roster.map((x, i) => notchRow(i + 1, x.sex, x.renn, x.lenn, rn, x.teats ?? '', x.weight ?? '', x.weights || null, x.tag ?? '')).join('');
  ...
  function genNotchRows(id) {
    ...
    body.innerHTML = generatedRows(b) || '<tr class="notch-empty"><td colspan="8">No live piglets in this batch — add rows manually if needed.</td></tr>';
  }
  ...
  function buildRoster(form, stage = null, typed = []) {
    ...
    return [...form.querySelectorAll('tr[data-notch-row]')].map((tr, ri) => {
      ...
      const row = {
        sex,
        tag: (tr.querySelector('[name="notch_tag"]')?.value || '').trim(), /* [FIX 136] */
      ...
      return row;
    }).filter(x => x.sex || x.renn || x.lenn || x.tag || x.weight !== '');
  }
